...
allmodels %>% 
  ...
  group_by(sp,k) %>% 
  summarise(n=n(),deltaAIC=min(deltaAIC)) %>%
...
write.csv(file="../output/tabla-AIC-modelos.csv",tableA1)
```
...
ggplot(data=dat) +
  ...
  annotate("rect", xmin = 0, xmax = 50, ymin = .5, ymax = 13.5,
  alpha = .2, lty=2, col="grey62") + 
  annotate("rect", xmin = 50, xmax = 125, ymin = 13.5, ymax = 23.5,
  alpha = .2, lty=2, col="grey62") + 
  annotate("rect", xmin = 125, xmax = 225, ymin = 23.5, ymax = 27.5,
  alpha = .2, lty=2, col="grey62") + 
...
- four widespread species predicted in more than 150 cells
- ten species with intermediate predictions (50 to 120 cells)
