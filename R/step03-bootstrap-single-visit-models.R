   ...
   if (!file.exists(out.file)) {
      if (nrow(sparams)>0) {
         ...
         if (nrow(slc_params)>0) {
            ...
            assign(sprintf("%s.full",sub(".rda","",sprda)),fit.full)
            fit.boot <- bootstrap(fit.full, B=99)
            fit.pboot <- bootstrap(fit.full, B=99, type="param") ## parametric bootstrap
            assign(sprintf("%s.boot",sub(".rda","",sprda)),fit.boot)
            assign(sprintf("%s.pboot",sub(".rda","",sprda)),fit.pboot)

            fit.step <- svocc.step(fit.full, model="sta")
            fit.null <- svocc(formula(nfml), data=pa.data[ss,],link.sta = lkf, link.det = "logit", penalized = FALSE, method = c( "optim"))
            assign(sprintf("%s.null",sub(".rda","",sprda)),fit.null)
            assign(sprintf("%s.step",sub(".rda","",sprda)),fit.step)

            rm(list=ls(pattern="fit."))
            save(file=out.file,list=ls(pattern=".boot|.pboot|.full|.null|.step"))
            rm(list=ls(pattern=".boot|.pboot|.full|.null|.step"))
